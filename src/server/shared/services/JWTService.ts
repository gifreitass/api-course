//o JWT não tem dados criptografados
//qnd recebemos um JWT, para verificarmos se ele é válido utilizamos a mesma chave que geramos para gerar
//e na outra parte estarão os dados que vamos passar para o usuário (ex: id), vamos passar para dentro do JWT

import * as jwt from 'jsonwebtoken'
//id do usuário, pois o JWT vai ser gerado exclusivamente para um usuário
interface IJwtData {
    uid: number
}

const sign = (data: IJwtData): string | 'JWT_SECRET_NOT_FOUND' => {
    if (!process.env.JWT_SECRET) {
        return 'JWT_SECRET_NOT_FOUND'
    }
    //parâmetros - payload: dados que desejamos jogar no jwt
    //secretOrPrivateKey: chave que não deve ser compartilhada
    //o jwt dura 24h
    return jwt.sign(data, process.env.JWT_SECRET, { expiresIn: '24h' })
}

const verify = (token: string): IJwtData | 'JWT_SECRET_NOT_FOUND' | 'INVALID_TOKEN' => {
    if (!process.env.JWT_SECRET) {
        return 'JWT_SECRET_NOT_FOUND'
    }

    try {
        const decoded = jwt.verify(token, process.env.JWT_SECRET)
        //se vier uma string é pq não conseguimos decodificar com sucesso
        if (typeof decoded === 'string') {
            return 'INVALID_TOKEN'
        }

        return decoded as IJwtData
    } catch (error) {
        return 'INVALID_TOKEN'
    }

}

export const JWTService = {
    sign,
    verify
}